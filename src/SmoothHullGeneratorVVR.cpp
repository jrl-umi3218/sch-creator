...
SmoothHullGeneratorVVR::SmoothHullGeneratorVVR(double r, double R) : _epsilon(1e-8), _ccw(false)
{
  _r = r;
  _R = R;
}
...
bool SmoothHullGeneratorVVR::findCenter(int p1, int p2, int p3, vector3d & center)
{
  ...
  {
#ifdef DISPLAY_INFO
    // std::cout << "WARNING, no sphere of radius " << _R << " can fit on face "  << p1 << " " << p2 << " " << p3 <<
    // std::endl;
#endif
    return false;
  }
  ...
  for(i = 0; i < n; ++i)
  {
    for(j = i + 1; j < n; ++j)
    {
      for(k = j + 1; k < n; ++k)
      {
        ...
        if(findCenter(i, j, k, c)) b = allPointsInSphere(c);
        if(b) break;
        // now let's test if every point is inside the sphere [i,k,j]
        if(findCenter(i, k, j, c)) b = allPointsInSphere(c);
        if(b)
        {
          _ccw = false;
          break;
...
double SmoothHullGeneratorVVR::distMaxPointsInSphere(vector3d & center)
{
  ...
  {
    std::cout << "PROBLEM, all points at center ? "
              << " center " << center << " nb points " << _points.size() << std::endl;
  }
...
void SmoothHullGeneratorVVR::cover(void)
{
  ...
  for(unsigned i = 0; i < n; ++i)
  {
    for(unsigned j = i + 1; j < n; ++j)
    {
      ...
      if(d_max < d.norm()) d_max = d.norm();
    }
  }
  std::cout << "Maximum body distance " << d_max << std::endl;
  if(d_max >= 2. * (_R - _r))
    std::cout << "ERROR, impossible to compute an STP-BV for this body with R=" << _R << ", choose larger R"
              << std::endl;
...
#else // find the triangle with maximum distance of other vertices inside, better but can be very time consuming for
      // large clouds
  int i_min = -1;
  int j_min = -1;
  int k_min = -1;
...
void SmoothHullGeneratorVVR::travelCover(VVRSFace s)
{
  ...
  postponed:
    ...
    {
      std::cout << "WARNING, no new vertex found in rotation around edge [" << td._p1 << ", " << td._p2
                << "] that satisfy inclusion in shpere of all other vertices with chosen precision" << std::endl;
    ...
    if(!sortedPoints.empty())
    {
      ...
      for(std::multimap<double, int>::iterator pi = sortedPoints.begin(); pi != sortedPoints.end(); ++pi)
      {
        std::cout << " " << pi->first << " " << pi->second << " "
                  << (_index.find(pi->second) != _index.end()); //->first;
        if(pi->second > 1.)
        {
          std::cout << " computed_edge_angle " << curr_edge_angle << " next "
                    << (edgeStackByAngle.begin())->first; //->first;
        }
      }
      std::cout << std::endl;
...
#endif
        while(getKey(it->second) != getKey(td1)) ++it;
...
#endif
        while(getKey(it->second) != getKey(td2)) ++it;
        edgeStackByAngle.erase(it);
...
void SmoothHullGeneratorVVR::printSphere(VVRSFace & s)
{
  ...
  std::cout << "   point1 [" << s._point1 << "] : " << _points[s._point1].x << ", " << _points[s._point1].y << ", "
            << _points[s._point1].z << std::endl;
  std::cout << "   point2 [" << s._point2 << "] : " << _points[s._point2].x << ", " << _points[s._point2].y << ", "
            << _points[s._point2].z << std::endl;
  std::cout << "   point3 [" << s._point3 << "] : " << _points[s._point3].x << ", " << _points[s._point3].y << ", "
            << _points[s._point3].z << std::endl;
}
...
void SmoothHullGeneratorVVR::output(const std::string & rootPath)
{
  ...
  if(output.is_open())
  {
    ...
    for(unsigned int i = 0; !_index.empty(); ++i)
    {
      if(*(_index.begin()) == i)
      {
        output << "[" << _points[*(_index.begin())].x << "," << _points[*(_index.begin())].y << ","
    ...
    {
      output << "[" << (map[_spheres[i]._point1] + 1) << "," << (map[_spheres[i]._point2] + 1) << ","
             << (map[_spheres[i]._point3] + 1) << "],";
    }
    output.close();
  }
}
...
void SmoothHullGeneratorVVR::computeVVR_Prime(const std::string & filename)
{
  ...
  for(unsigned(i) = 0; i < _points.size(); ++i)
  {
    ...
    for(unsigned j = 0; j < _points.size(); ++j) computed[i][j] = false;
  }

  for(std::vector<vector3d>::const_iterator it = _points.begin(); it != _points.end(); ++it)
  {
    stmp._center = *it;
    stmp._radius = _r;
  ...
  int indexPrinting = 0;
  for(std::vector<VVRSFace>::const_iterator it = _spheres.begin(); it != _spheres.end(); ++it)
  {
      ...
      it != tVVR.end(); ++it)
  {
    ...
    if((bigSpheres[firstID]._center - bigSpheres[secondID]._center).normsquared() < epsilon)
    {
      ...
      linkedbSphereIDs.insert(std::pair<int, std::pair<int, int>>(
          it->second.second.first.first, std::pair<int, int>(currentTorusID, it->second.second.second.first)));
      linkedbSphereIDs.insert(std::pair<int, std::pair<int, int>>(
          it->second.second.second.first, std::pair<int, int>(currentTorusID, it->second.second.first.first)));
    }
  }
  ...
  for(std::multimap<int, int>::iterator it = linkedsSphereIDs.begin(); it != linkedsSphereIDs.end(); ++it)
  {
    ...
    if(it2 != ssVVR[it->first].end()) ssVVR[it->first].erase(it2);
  }
  ...
  for(std::multimap<int, std::pair<int, int>>::iterator it = linkedbSphereIDs.begin(); it != linkedbSphereIDs.end();
      ++it)
  {
  ...
  for(std::map<int, bool>::reverse_iterator it = torusToEraseID.rbegin(); it != torusToEraseID.rend(); ++it)
  {
    for(std::vector<std::vector<VVRcone>>::iterator it2 = ssVVR.begin(); it2 != ssVVR.end(); ++it2)
    {
      ...
      {
        if(it3->first > it->first) --(it3->first);
      }
    }
    for(std::vector<faceVVR>::iterator it2 = bsVVR.begin(); it2 != bsVVR.end(); ++it2)
    {
      if(it2->_plane1.first > it->first) --(it2->_plane1.first);
      if(it2->_plane2.first > it->first) --(it2->_plane2.first);
      if(it2->_plane3.first > it->first) --(it2->_plane3.first);
    }
  }
  ...
  {
    os << smallSpheres[*it]._radius << " " << smallSpheres[*it]._center.x << " " << smallSpheres[*it]._center.y << " "
       << smallSpheres[*it]._center.z << std::endl;
  ...
  for(std::vector<SCD::VVRsphere>::const_iterator it = bigSpheres.begin(); it != bigSpheres.end(); ++it)
  {
    ...
    // face vertices
    os << _points[_spheres[i]._point1].x << " " << _points[_spheres[i]._point1].y << " "
       << _points[_spheres[i]._point1].z << " ";
    os << _points[_spheres[i]._point2].x << " " << _points[_spheres[i]._point2].y << " "
       << _points[_spheres[i]._point2].z << " ";
    os << _points[_spheres[i]._point3].x << " " << _points[_spheres[i]._point3].y << " "
       << _points[_spheres[i]._point3].z << std::endl;

    // VVR
    os << bsVVR[i]._plane1.first - difference << " ";
    os << bsVVR[i]._plane1.second.x << " " << bsVVR[i]._plane1.second.y << " " << bsVVR[i]._plane1.second.z
       << std::endl;
    os << bsVVR[i]._plane2.first - difference << " ";
    os << bsVVR[i]._plane2.second.x << " " << bsVVR[i]._plane2.second.y << " " << bsVVR[i]._plane2.second.z
       << std::endl;
    os << bsVVR[i]._plane3.first - difference << " ";
    os << bsVVR[i]._plane3.second.x << " " << bsVVR[i]._plane3.second.y << " " << bsVVR[i]._plane3.second.z
       << std::endl;
  ...
  /*for(int i = 0 ; i < smallSpheres.size() ; ++i)
    ...
    os << bsVVR[i]._plane1.second.x << " " << bsVVR[i]._plane1.second.y << " " << bsVVR[i]._plane1.second.z <<
    std::endl; os << (smallSpheres.size() + bigSpheres.size() + bsVVR[i]._plane2.first) << " "; os <<
    bsVVR[i]._plane2.second.x << " " << bsVVR[i]._plane2.second.y << " " << bsVVR[i]._plane2.second.z << std::endl; os
    << (smallSpheres.size() + bigSpheres.size() + bsVVR[i]._plane3.first) << " "; os << bsVVR[i]._plane3.second.x << " "
    << bsVVR[i]._plane3.second.y << " " << bsVVR[i]._plane3.second.z << std::endl;
    }
...
void SmoothHullGeneratorVVR::computeVVR(const std::string & filename)
{
  ...
  {
    computed[i].resize(_points.size());
    for(int j = 0; j < _points.size(); ++j) computed[i][j] = false;
  }
...
void SmoothHullGeneratorVVR::computeVVR_WithPolyhedron(const std::string & filename)
{
  ...
  for(std::vector<int>::const_iterator it = ind.begin(); it != ind.end(); ++it)
  {
    os << smallSpheres[*it]._center.x << " " << smallSpheres[*it]._center.y << " " << smallSpheres[*it]._center.z
       << std::endl;
  }
  os.close();

  // spawnl(P_WAIT,"qconvex.exe","qconvex.exe",(std::string(" TI ")+filename+std::string(".inp")).c_str(),(std::string("
  // TO ")+filename+std::string(".otp")).c_str()," Qt"," o"," f",NULL);//call to qconvex.exe
}
} // namespace SCD
